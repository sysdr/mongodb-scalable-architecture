...
if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^${MONGO_CONTAINER_NAME}$"; then
    ...
    docker run -d --name "${MONGO_CONTAINER_NAME}" -p "${MONGO_PORT}:27017" mongo:8.0-rc-jammy
...
# Run demo/benchmark so dashboard metrics are non-zero
echo "[INFO] Running benchmark (demo) to populate metrics..."
node "${NODE_APP_FILE}" || { echo "[WARN] Benchmark had errors; continuing with dashboard."; }
...
echo ""
echo "[SUCCESS] Start complete. To stop MongoDB, run: ${SCRIPT_DIR}/stop.sh"
