...
[[ -f "${PROJECT_DIR}/index.js" ]] || fail "Missing: index.js"
...
[[ -f "${PROJECT_DIR}/cleanup.sh" ]] || fail "Missing: cleanup.sh"
...
if [[ "$mongo_reachable" -eq 1 ]]; then
    for cmd in mongosh mongo; do
        if command -v "$cmd" &>/dev/null; then
            n=$("$cmd" --host localhost --port "${MONGO_PORT}" --quiet --eval "db.getSiblingDB('idhackDB').contentItems.countDocuments()" 2>/dev/null || echo 0)
            [[ "${n:-0}" -gt 0 ]] && pass "idhackDB.contentItems has ${n} document(s) (dashboard non-zero)" || echo "[INFO] idhackDB.contentItems empty; run demo.sh or start.sh"
            break
        fi
    done
else
