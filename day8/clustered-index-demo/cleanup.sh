...
# remove node_modules, venv, .pytest_cache, .pyc, Istio from this project.
set -e
...
echo "[INFO] Stopping and removing container '${MONGO_CONTAINER_NAME}' (if present)..."
docker stop "${MONGO_CONTAINER_NAME}" 2>/dev/null || true
docker rm "${MONGO_CONTAINER_NAME}" 2>/dev/null || true

echo "[INFO] Pruning unused Docker resources (containers, images, networks, volumes)..."
docker system prune -af --volumes 2>/dev/null || true

echo "[INFO] Removing node_modules, venv, .pytest_cache, .pyc, Istio from project..."
